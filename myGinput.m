% Edited by Jacob Bakermans, January 2016
function varargout = myGinput(arg1)
...
% end
    ...
    while how_many ~= 0
%         try
%             selectionMode = waitForUserInput(fig);
%         catch %#ok<CTCH>
%             cleanup(c);
%             if(ishghandle(fig))
...
function selectionMode = waitForUserInput(fig)
waitfor(fig,'UserData')
% Extract selectionMode to determine if key or mouse was used
selectionMode = get(fig,'UserData'); 
if ischar(selectionMode)
    ud = strsplit(selectionMode, '_');
    selectionMode = ud{1};
...
figWidth = figPos(3);
figHeight = figPos(4);
cp(1) = max(1, min(figWidth, cp(1)));
cp(2) = max(1, min(figHeight, cp(2)));
