...
function newMols = loadMolsSingleIntensity(in, currentMols)
    % Initiate variables
    ...
    for (currentFile = 1:numberOfFiles)        
        ...
        if (numberOfMolecules > length(newMols)) % If these molecules are new
            for (i = 1:molsInFile)
                ...
                data = allMolData(i);
                DD = 1-data.intensity(:,1)./5000; % data.intensity(2:2:end,1);
                DA = data.intensity(:,1)./5000; % data.intensity(2:2:end,2);
                AD = data.intensity(:,1); % data.intensity(1:2:end,1);
                AA = data.intensity(:,1); % data.intensity(1:2:end,2);
                E = data.intensity(:,1);
                S = data.intensity(:,1);
                % Save molecule data
